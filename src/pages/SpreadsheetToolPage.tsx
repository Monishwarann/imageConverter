...
interface SpreadsheetToolPageProps {
  queue: FileItem[];
  selectedIds: string[];
...
export const SpreadsheetToolPage: React.FC<SpreadsheetToolPageProps> = (props) => {
  const sheetQueue = props.queue.filter((q) => q.category === 'spreadsheet');
    ...
    <div className="flex flex-col gap-6">
      <div className="glass-panel p-6 rounded-3xl border border-slate-800 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-white">Excel & Spreadsheet Suite</h2>
            <p className="text-xs text-slate-400">Convert XLSX, XLS, CSV, TSV & ODS to Excel, CSV, JSON or PDF tables.</p>
          </div>
        </div>
      ...
      <UniversalUploadZone onFilesAdded={props.onFilesAdded} activeCategory="spreadsheet" />
